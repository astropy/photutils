...
"""
Monte Carlo verification of the SourceCatalog centroid errors.

This is a validation script rather than a timing benchmark. It checks
the reported pixel and sky centroid errors of ``SourceCatalog``
against the scatter of the measured centroids over many noise
realizations.
...
* the covariance transport alone: pixel positions are drawn from a
  ...
  the errors that ``SourceCatalog`` transports to the sky with the local
  WCS Jacobian, for several WCS types

* end to end: a Gaussian source is simulated many times with Gaussian
  noise, measured with ``SourceCatalog`` given an ``error`` array and a
  sheared, anisotropic WCS, and the scatter of the pixel and sky
  centroids is compared with the mean reported errors
...
computing moments, which makes the centroid a clipped estimator when
the segment extends into wings below the noise. The reported error,
a linear propagation, is then a few percent high. Use a footprint
whose edge is well above the noise (the defaults) to see agreement, or
``--radius 8`` with the default amplitude to see the effect.

Run ``python benchmarks/mc_sky_centroid_err.py --help`` to see the
available options.
"""
...
def check_end_to_end(*, amplitude=400.0, radius=7.0, n_real=4000, seed=0):
    """
    Compare the reported centroid errors with the scatter of measured
    centroids over noise realizations.
    ...
    for i in range(n_real):
        ...
        skycoord = cat.sky_centroid[0]
        results[i] = (cat.x_centroid[0], cat.y_centroid[0],
                      skycoord.ra.deg, skycoord.dec.deg,
                      cat.x_centroid_err[0], cat.y_centroid_err[0],
                      cat.sky_centroid_ra_err[0].to_value(u.arcsec),
                      cat.sky_centroid_dec_err[0].to_value(u.arcsec))
    xcen, ycen, ra, dec, xerr, yerr, raerr, decerr = results.T
    ...
    print(f'{"quantity":>20}{"MC scatter":>12}{"reported":>12}{"ratio":>8}')
    rows = [('x (pix)', xcen.std(ddof=1), xerr.mean()),
            ('y (pix)', ycen.std(ddof=1), yerr.mean()),
            ('East (arcsec)', east.std(ddof=1), raerr.mean()),
            ('North (arcsec)', north.std(ddof=1), decerr.mean())]
    for label, mc, reported in rows:
        print(f'{label:>20}{mc:12.5f}{reported:12.5f}{mc / reported:8.4f}')
    precision = 1.0 / np.sqrt(2.0 * n_real)
    print(f'Monte Carlo precision of a standard deviation: {precision:.4f}.')


def main():
    """
    Run the SourceCatalog centroid error checks.
    """

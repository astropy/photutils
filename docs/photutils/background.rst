...
Photutils provides the :class:`~photutils.background.Background` and
:class:`~photutils.background.BackgroundIDW` classes to estimate the
2D background and background rms noise in an astronomical image.  To
resize the low-resolution maps,
:class:`~photutils.background.Background` uses bicublic spline
interpolation, while :class:`~photutils.background.BackgroundIDW` uses
inverse-distance weighted interpolation (see
...
a box size of 50x50, a 3x3 median filter, and the "median" background
...
.. doctest-requires:: scipy
    ...
    >>> bkg = Background(data2, (50, 50), filter_size=(3, 3), method='median')
...
.. doctest-requires:: scipy
    ...
    >>> mask = (data3 == 0)
    >>> bkg3 = Background(data3, (25, 25), filter_size=(3, 3), mask=mask)

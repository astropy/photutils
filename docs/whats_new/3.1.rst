...
`~photutils.psf.GriddedPSFModel` evaluations are now significantly
faster, with typical performance improvements of approximately 20–25%.
The optimizations reduce the overhead of PSF interpolation, improving
the runtime of PSF photometry workflows.
...
The ``segmentation_image`` must be a
`~photutils.segmentation.SegmentationImage` (see :ref:`Image
Segmentation <image_segmentation>`) that labels each source with a
positive integer, and the ``mask_method`` keyword selects how
neighboring sources are handled:
...
* ``'source_only'``: only pixels belonging to the target source are
  included.
* ``'background_only'``: pixels belonging to any source are excluded,
  e.g., to estimate the local background within an annulus aperture.
...
It is required to explicitly provide the source ``labels``
for each aperture when a ``segmentation_image`` is input and
``mask_method`` is ``'mask'``, ``'source_only'``, or ``'correct'`` (the
``'background_only'`` method does not use the labels). The ``labels``
keyword must have the same length as the number of aperture positions
and identifies the target source for each aperture. Requiring explicit
labels ensures the correct target source is used for each aperture,
which is especially important for irregularly shaped source segmentation
footprints or for apertures that are not centered on a source.
...
* sources whose net flux is not positive or whose second-order
  moments are not positive semidefinite (``'undefined_shape'``),
  whose covariance matrix is singular or nearly singular
  (``'singular_covariance'``), or whose centroid lies outside the
  aperture bounding box (``'centroid_outside'``), so that the
  centroid and shape properties are undefined or unreliable, in
...
Sky Orientation of Sources
==========================

`~photutils.segmentation.SourceCatalog` and
`~photutils.aperture.ApertureStats` have new
`~photutils.segmentation.SourceCatalog.sky_orientation` and
`~photutils.aperture.ApertureStats.sky_orientation` properties
giving the position angle on the sky of the major axis of
each source, measured from North toward East in the range
(-90, 90] degrees. It is computed by transporting the pixel
`~photutils.segmentation.SourceCatalog.covariance` matrix to the local
tangent plane with the WCS Jacobian at each source centroid, so it
accounts for the WCS rotation, parity, and local distortion::
    ...
    >>> segm = detect_sources(data, 10.0, n_pixels=10)
    >>> wcs = make_wcs(data.shape)
    >>> cat = SourceCatalog(data, segm, wcs=wcs)
    >>> print(cat.orientation.to_value('deg').round(2))
    [70.49]
    >>> print(cat.sky_orientation.to_value('deg').round(2))
    [40.49]
...
Source deblending with :func:`~photutils.segmentation.deblend_sources`
and `~photutils.segmentation.SourceFinder` is now typically 7--50 times
faster, producing identical results. The whole watershed step now runs
in compiled code.

The compiled watershed kernel produces results identical to
``skimage.segmentation.watershed``, so scikit-image is no longer an
optional dependency.
...
computation. Each chunk of sources is deblended by a few compiled calls
that release the GIL, so multithreading also speeds up fields of many
small sources.

A new ``contrast_method`` keyword selects the flux used by the
deblending contrast criterion. The ``'basin'`` method preserves the
existing behavior where the contrast fraction is the total flux
...
never slower and is substantially faster when many below-contrast basins
would otherwise be removed iteratively. The default of `None` currently
resolves to ``'basin'``. The default may change to ``'saddle'`` in
version 4.0.
...
All Photutils deprecation warnings are now emitted as
`~photutils.utils.PhotutilsDeprecationWarning`, a subclass of astropy's
`~astropy.utils.exceptions.AstropyDeprecationWarning`. The warning class
name now identifies Photutils as the source of the warning, which makes
it easier to find the deprecated code path when a warning is raised from
...
`~photutils.psf.EPSFBuilder` no longer produces a checkerboard pattern
in the ePSF when the input stars are heterogeneous. This includes stars
...
Circular Apertures as Ellipses
==============================

A circle on the sky maps to an ellipse in pixel coordinates whenever
the pixels are not square or the WCS is sheared, and likewise a
pixel circle maps to an ellipse on the sky. The ``to_pixel`` and
``to_sky`` methods of `~photutils.aperture.CircularAperture`,
`~photutils.aperture.CircularAnnulus`,
`~photutils.aperture.SkyCircularAperture`, and
`~photutils.aperture.SkyCircularAnnulus` now accept an ``as_ellipse``
keyword. If `True`, the image of the circle under the local linear
WCS at the reference position is returned as an ellipse instead of a
circle scaled by the mean pixel scale. The default remains a circle, so
existing results are unchanged::
    ...
    >>> wcs.wcs.cdelt = [-0.05 / 3600, 0.03 / 3600]  # non-square pixels
    >>> aperture = SkyCircularAperture(SkyCoord(150.0, 20.0, unit='deg'),
    ...                                r=1 * u.arcsec)
    >>> ellipse = aperture.to_pixel(wcs, as_ellipse=True)
    >>> repr(ellipse)
    <EllipticalAperture([49., 49.], a=33.33333336112715, b=20.000000013646787, theta=90.00000000582446 deg)>

For most imagers the pixel anisotropy is a percent or less, and the flux
difference between the ellipse and the mean-scale circle is negligible.
The ellipse matters for strongly rectangular pixels or a strongly
sheared WCS, and whenever the footprint of the aperture must follow
the WCS rather than the mean pixel scale.


Per-Pixel Sky Areas from a WCS
==============================

The new :func:`~photutils.utils.compute_pixel_areas` and
:func:`~photutils.utils.compute_pixel_area_map` functions compute the
on-sky area of image pixels directly from a WCS, either at an array of
positions or for every pixel in an image. The area of each pixel is the
absolute determinant of the local WCS Jacobian, evaluated with central
finite differences, so it follows the distortion across the field and is
well defined at the celestial poles and across the RA wraparound. Both
functions accept any WCS that supports the astropy shared WCS interface,
including ``gwcs`` objects.

The pixel area of a distorted image typically varies by a few percent
across the field. Converting a surface-brightness image (e.g., in
MJy/sr) to fluxes with a single nominal pixel area therefore leaves a
position-dependent error. Multiplying by the pixel-area map removes it::
    ...
    >>> shape = (100, 100)
    >>> wcs = make_wcs(shape)
    >>> areas = compute_pixel_area_map(wcs, shape) * u.arcsec**2
    >>> areas[50, 50]
    <Quantity 0.01 arcsec2>
    >>> pixel_area_sr = areas.to(u.sr)  # multiply a MJy/sr image by this

The map is interpolated from a coarse grid of positions by default,
which is accurate to better than a part per million for typical
distortions. Pass ``interpolate=False`` to evaluate the area exactly at
every pixel instead.
...
The ``PixelAperture.do_photometry`` method is now
deprecated and will be removed in version 4.0. Use the new
`~photutils.aperture.AperturePhotometry` class instead.

Deblending ``progress_bar`` and ``n_processes`` Deprecated
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The ``progress_bar`` and ``n_processes`` keywords of
:func:`~photutils.segmentation.deblend_sources` and
`~photutils.segmentation.SourceFinder` are deprecated and will be
removed in version 4.0, and both keywords no longer have any effect.
...
``EPSFBuildResult`` name is deprecated and will be removed in version
4.0.
...
Covariance Regularization of Unresolved Sources
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

The ``covariance`` matrix of
:class:`~photutils.segmentation.SourceCatalog` and
:class:`~photutils.aperture.ApertureStats` is regularized for unresolved
sources, because a source cannot have a second moment smaller than
``1/12`` pixel**2, the variance of a uniform distribution across a
single pixel. Previously, ``1/12`` was added to both diagonal elements
when the covariance determinant was below ``(1/12)**2``. It misses a
thin source that is unresolved along only its minor axis if its major
axis is long, it inflates the variance along a resolved axis, and it is
discontinuous.

The variance along each principal axis (each eigenvalue of the
covariance matrix) is now raised to at least ``1/12`` while the
orientation is kept. A resolved axis is not modified, and the result
is a continuous function of the image moments. The sources that are
regularized are exactly those with the ``singular_covariance`` flag.
Sources whose second-order moments are not positive semidefinite have
NaN shape properties and the ``undefined_shape`` flag instead. A source
that is unresolved along both axes becomes isotropic with an orientation
of zero. An exactly thin tilted source, whose covariance determinant is
zero apart from rounding, is now regularized instead of having NaN shape
properties.

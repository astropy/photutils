...
and `~photutils.segmentation.SourceFinder` is now typically 5--30 times
faster, producing identical results. The whole watershed step now runs
in compiled code. As an example, deblending a field of 2000 blended
source pairs dropped from about 1.6 s to 0.05 s.

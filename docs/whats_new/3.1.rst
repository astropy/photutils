...
whose meaning is unchanged, apart from flagged sources whose
second-order moments are not positive semidefinite, which have NaN
shape properties. A source that is unresolved along both axes becomes
isotropic with an orientation of zero. An exactly thin tilted source,
whose covariance determinant is zero apart from rounding, is now
regularized instead of having NaN shape properties.

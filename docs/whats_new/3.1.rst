...
:func:`~photutils.utils.pixel_area_map` functions compute the on-sky
area of image pixels directly from a WCS, either at an array of
...
position-dependent error. Multiplying by the pixel-area map removes it::
    ...
    >>> shape = (100, 100)
    >>> wcs = make_wcs(shape)
    >>> areas = pixel_area_map(wcs, shape) * u.arcsec**2
    >>> areas[50, 50]

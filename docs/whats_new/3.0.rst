...
instabilities when the source PSF has very small Fourier coefficients::
    ...
    >>> kernel = make_kernel(source_psf, target_psf,
    ...                                 otf_threshold=1e-4)
...
function has been added to the ``psf_matching`` subpackage. This
function creates PSF matching kernels using Wiener (Tikhonov)
regularization, which provides an alternative to the traditional
Fourier-ratio approach with hard amplitude thresholding used by
:func:`~photutils.psf_matching.make_kernel`. The Wiener regularization
approach suppresses noise amplification at spatial frequencies where
the source response is weak by adding a regularization term to the
denominator.

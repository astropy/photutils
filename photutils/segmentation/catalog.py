...
class SourceCatalog:
    ...
    **Scalar vs. Multi-source Catalogs**
    ...
    def sky_orientation(self):
        ...
        `centroid`, which accounts for the WCS rotation, parity, and
        ...
        if self.wcs is None:
            return self._null_objects
        sky_cov = self._sky_cov_from_pixel_cov(self._covariance,
                                               self._array('centroid'))

        # The tangent-plane axes are ordered (East, North). Measuring
        # the angle from North toward East makes North play the role
        # of the x axis and East the role of the y axis in the pixel
        # `orientation` formula.
    ...
    def _calc_circular_photometry(self, radius):
        ...
        # A pixel is a member if it is inside the aperture and not
        # excluded by the input mask, non-finite data, or the
    ...
    def _calc_kron_photometry(self, *, kron_params=None):
        ...
        for idx, psrc, shape_code in (
            ...
            # A pixel is a member if it is inside the aperture and not
            # excluded by the input mask, non-finite data, or the
            # segmentation masking. Under 'correct', uncorrectable

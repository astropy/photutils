...
apertures = {'edge': CircularAperture([(0, 0)], r=2.0),
             ...
             'multiple_objects': CircularAperture([(5, 5), (10, 13)],
                                                  r=2.0)}
            ...
            if xcenter < img_dim[0] and ycenter < img_dim[1]:
                for xpix in range(int(xcenter) - 3, int(xcenter) + 4):
                    for ypix in range(int(ycenter) - 3, int(ycenter) + 4):
                        img[xpix, ypix] = 5 - max(abs(xcenter - xpix),
                                                  abs(ycenter - ypix))
    return img
...
def test_aperture_edge():
    ...
    assert (np.allclose(test_flux, phot_flux, rtol=0.01),
            "Edge case failed, test flux= {}, phot flux = {}".format(
                test_flux, phot_flux))
...
def test_basic_case():
    ...
    assert (np.allclose(test_flux, phot_flux, rtol=0.01),
            "Full overlap case failed, test flux= {}, phot flux = {}".format(
                test_flux, phot_flux))


def test_fractional_pixel_center():
    test_img = insert_source(apertures.values())
    test_flux, phot_flux = aperture_test(apertures['fractional_pixel'],
                                         test_img)
    assert (np.allclose(test_flux, phot_flux, rtol=0.01),
            "fractional pixel case failed, test flux= {}, phot "
            "flux = {}".format(test_flux, phot_flux))


def test_diff_radius():
    test_img = insert_source(apertures.values())
    test_flux, phot_flux = aperture_test(apertures['different_radius'],
                                         test_img)
    assert (np.allclose(test_flux, phot_flux, rtol=0.01),
            "different radius case failed, test flux= {}, phot "
            "flux = {}".format(test_flux, phot_flux))


def test_multiple_objects():
    test_img = insert_source(apertures.values())
    test_flux, phot_flux = aperture_test(apertures['multiple_objects'],
                                         test_img)
    assert (np.allclose(test_flux, phot_flux, rtol=0.01),
            "multiple objects case failed, test flux= {}, phot "
            "flux = {}".format(test_flux, phot_flux))

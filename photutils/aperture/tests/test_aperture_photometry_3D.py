...
from ...datasets import (get_path, make_4gaussians_image, make_wcs,
                         make_imagehdu, make_4gaussians_cube, make_cubehdu)
...
def test_outside_array(aperture_class, params):
    ...
    assert np.all(np.isnan(fluxtable['aperture_sum']))
...
class BaseTestAperturePhotometry(object):
    ...
    def test_scalar_error(self):
        ...
        if not isinstance(self.aperture, (RectangularAperture,
                                          RectangularAnnulus)):
            [assert_allclose(tab3_ap_sum_Col_i, true_flux_i)
             for (tab3_ap_sum_Col_i, true_flux_i)
             in zip(np.atleast_2d(table3['aperture_sum']), np.atleast_1d(self.true_flux))]
            assert_allclose(table2['aperture_sum'], table3['aperture_sum'],
                            atol=0.1)
        assert np.all(table1['aperture_sum'] < table3['aperture_sum'])
        ...
        if not isinstance(self.aperture, (RectangularAperture,
                                          RectangularAnnulus)):
            [assert_allclose(tab3_ap_sum_err_Col_i, true_flux_i)
             for (tab3_ap_sum_err_Col_i, true_flux_i)
             in zip(np.atleast_2d(table3['aperture_sum_err']), np.atleast_1d(true_error))]
            assert_allclose(table2['aperture_sum'], table3['aperture_sum'],
                            atol=0.1)
...
class TestCircular(BaseTestAperturePhotometry):
    ...
    def setup_class(self):
        self.data = np.ones((100, 40, 40), dtype=np.float)
        position = (20., 20.)
        r = 10.
...
class TestMaskedSkipCircular(BaseTestAperturePhotometry):
    ...
    def setup_class(self):
        self.data = np.ones((100, 40, 40), dtype=np.float)
        self.mask = np.zeros((100, 40, 40), dtype=bool)
...
class TestInputHDUList(BaseTestDifferentData):
    ...
    def setup_class(self):
        data0 = np.ones((100, 40, 40), dtype=np.float)
        data1 = np.empty((100, 40, 40), dtype=np.float)
        data1.fill(2)
...
def test_wcs_based_photometry():
    data = make_4gaussians_cube()
    wcs = make_wcs(data.shape)
    hdu = make_cubehdu(data, wcs=wcs)

    # hard wired positions in make_4gaussians_image
    ...
    assert_allclose(np.atleast_2d(photometry_skycoord_circ['aperture_sum'][2]),
                    np.atleast_2d(photometry_skycoord_circ_s['aperture_sum']))
    ...
    assert_allclose(np.atleast_2d(photometry_skycoord_circ_ann['aperture_sum'][2]),
                    np.atleast_2d(photometry_skycoord_circ_ann_s['aperture_sum']))
    ...
    assert_allclose(np.atleast_2d(photometry_skycoord_ell['aperture_sum'][2]),
                    np.atleast_2d(photometry_skycoord_ell_s['aperture_sum']))

    assert_allclose(np.atleast_2d(photometry_skycoord_ell_ann['aperture_sum'][2]),
                    np.atleast_2d(photometry_skycoord_ell_ann_s['aperture_sum']))
    ...
    assert_allclose(np.atleast_2d(photometry_skycoord_rec['aperture_sum'][2]),
                    np.atleast_2d(photometry_skycoord_rec_s['aperture_sum']))

    assert np.all(photometry_skycoord_rec['aperture_sum'] >
                  photometry_skycoord_circ['aperture_sum'])

    assert_allclose(np.atleast_2d(photometry_skycoord_rec_ann['aperture_sum'][2]),
                    np.atleast_2d(photometry_skycoord_rec_ann_s['aperture_sum']))
...
def test_basic_circular_aperture_photometry_unit():
    data1 = np.ones((100, 40, 40), dtype=np.float)
...
def test_aperture_partial_overlap():
    data = np.ones((100, 20, 20))
    error = np.ones((100, 20, 20))

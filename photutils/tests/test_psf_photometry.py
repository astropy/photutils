...
import numpy as np
from photutils.psf import create_psf, DiscretePSF
...
test_psf = discretize_model(psf_model, (0, psf_size), (0, psf_size), mode='center')
...
def test_create_psf():
    """
    Check if create_psf works correctly on simulated data.
    """
    psf = create_psf(image, positions, psf_size)
    assert np.all(np.abs(psf - test_psf) < 0.02)
...
def test_create_psf_flux():
    ...
    """
    psf = create_psf(image, positions, psf_size, fluxes)
    assert np.abs(psf.sum() - 1) < 0.001
    assert np.all(np.abs(psf - test_psf) < 0.1)

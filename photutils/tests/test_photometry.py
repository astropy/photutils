...
def test_annulus_ell_basic():
    from ..photometry import annulus_ell
    import numpy as np
...
def test_annulus_ell_ones():
    ...
    b_out = 5.
    theta = np.pi / 4.
    flux = annulus_ell(arr, 20., 20., a_in, a_out, b_out, theta)
    ...
    b_out = 5.
    theta = np.pi / 4.
    flux = annulus_ell(arr, 60., 60., a_in, a_out, b_out, theta)
    true_flux = np.pi * (a_out * b_out - a_in ** 2 * b_out / a_out)

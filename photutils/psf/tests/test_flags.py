...
def test_decode_psf_flags():
    ...
    all_flags = 1 + 2 + 4 + 8 + 16 + 32 + 64 + 128 + 256 + 512 + 2048  # 3071
    ...
    expected_all = ['npixfit_partial', 'outside_bounds', 'negative_flux',
                    ...
                    'non_finite_position', 'non_finite_localbkg']
...
def test_psf_flags_properties():
    ...
    expected_bits = [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 2048]
    assert set(bit_values) == set(expected_bits)
    assert len(bit_values) == 11
...
def test_psf_flags_completeness():
    ...
    assert len(PSF_FLAGS.all_flags) == 11
